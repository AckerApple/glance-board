...
function screenDataPanel(card: DisplayCard) {
  const parsed = parsedDataForCard(card);
  const summaryItems = dataSummaryItems(card, parsed);
  ...
  return div.class`screen-data`(
    ...
    details.attr("open", "")(
      ...
      pre.class`data-json`(formatJson(parsed))
    ),
    card.debug ? details(
      summary("Provider debug"),
      pre.class`data-json`(formatJson(card.debug))
    ) : null,
    card.raw !== undefined ? details(
      summary("Raw source data"),
      pre.class`data-json`(formatJson(card.raw))
    ) : null
  );
}
